    ...
    public function upload($path='', $name='')
    {
        ...
        if(is_array($this->_files[$sign]['name'])){
            foreach ($this->_files[$sign]['name'] as $num => $value) {
            	...
            	$temp = explode($_SERVER['DOCUMENT_ROOT'], $this->_uploadOne($nameNew, $sign, $num));
            	$this->uploadedFiles[] = $temp[1];
            }
        }else{
        	if(is_array($name)){
                $name = $name[0];
        	}
        	$temp = explode($_SERVER['DOCUMENT_ROOT'], $this->_uploadOne($name, $sign));
            $this->uploadedFiles = $temp[1];
